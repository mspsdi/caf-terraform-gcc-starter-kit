...
resource "azurerm_api_management" "apim" {
  ...
  # stv2 requirement - public_ip_address_id
  # public_ip_address_id          = try(local.ip_configuration[var.settings.public_ip_key].ip_address_id, null)

  public_ip_address_id = try(coalesce(
    try(var.public_ip_addresses[var.client_config.landingzone_key][var.settings.public_ip_key].id, var.public_ip_addresses[var.settings.lz_key][var.settings.public_ip_key].id, null),
    try(var.settings.public_ip_id, null)
  ), null)

  # public_ip_address_id          = var.settings.public_ip_address_id # try(local.ip_configuration[frontend_ip_configuration.key].ip_address_id, null)
