...
resource "azurerm_container_group" "acg" {
  ...
  subnet_ids = try(var.settings.network.subnets, null) == null ? null : [
    ...
    for key, value in var.settings.network.subnets : can(value.subnet_id) || can(value.virtual_subnet_key) ? try(value.subnet_id, var.virtual_subnets[try(value.lz_key, var.client_config.landingzone_key)][value.virtual_subnet_key].id) : var.combined_resources.networking[try(value.lz_key, var.client_config.landingzone_key)][value.vnet_key].subnets[value.subnet_key].id
  ]
