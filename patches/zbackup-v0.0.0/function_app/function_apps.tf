module "function_app" {
  ...
  settings            = each.value
  resource_id         = can(each.value.resource_id) ? each.value.resource_id : var.remote_objects.function_apps[var.client_config.landingzone_key][try(each.value.key, each.key)].id
  subresource_names   = toset(try(each.value.private_service_connection.subresource_names, ["sites"]))
...
module "function_app_remote" {
  ...
  client_config       = var.client_config
  settings            = each.value
  resource_id         = can(each.value.key) ? var.remote_objects.function_apps[each.value.lz_key][each.value.key].id : var.remote_objects.function_apps[each.value.lz_key][each.key].id
  subresource_names   = toset(try(each.value.private_service_connection.subresource_names, ["sites"]))
  subnet_id           = var.subnet_id
  # subnet_id           = can(each.value.subnet_id) ? each.value.subnet_id : var.vnets[try(each.value.lz_key, var.client_config.landingzone_key)][each.value.vnet_key].subnets[each.value.subnet_key].id
  private_dns         = var.private_dns
