...
resource "azurerm_logic_app_standard" "logic_app_standard" {
  ...
  virtual_network_subnet_id = can(var.vnet_integration.subnet_id) ? var.vnet_integration.subnet_id : try(var.vnets[try(var.vnet_integration.lz_key, var.client_config.landingzone_key)][var.vnet_integration.vnet_key].subnets[var.vnet_integration.subnet_key].id,
...
# TODO: remove since this is not using
