...
resource "azurerm_kubernetes_cluster" "aks" {
  ...
  default_node_pool {
    ...
    pod_subnet_id  = can(var.settings.default_node_pool.pod_subnet_key) == false || can(var.settings.default_node_pool.pod_subnet.key) == false || can(var.settings.default_node_pool.pod_subnet_id) || can(var.settings.default_node_pool.pod_subnet.resource_id) || can(var.settings.default_node_pool.pod_subnet.virtual_subnet_key) ? try(var.settings.default_node_pool.pod_subnet_id, var.settings.default_node_pool.pod_subnet.resource_id, var.virtual_subnets[try(var.settings.default_node_pool.pod_subnet.lz_key, var.client_config.landingzone_key)][var.settings.default_node_pool.pod_subnet.virtual_subnet_key].id, null) : var.vnets[try(var.settings.lz_key, var.client_config.landingzone_key)][var.settings.vnet_key].subnets[try(var.settings.default_node_pool.pod_subnet_key, var.settings.default_node_pool.pod_subnet.key)].id
    ...
    vnet_subnet_id = can(var.settings.default_node_pool.vnet_subnet_id) || can(var.settings.default_node_pool.subnet.resource_id) || can(var.settings.default_node_pool.virtual_subnet_key) ? try(var.settings.default_node_pool.vnet_subnet_id, var.settings.default_node_pool.subnet.resource_id, var.virtual_subnets[try(var.settings.default_node_pool.lz_key, var.client_config.landingzone_key)][var.settings.default_node_pool.virtual_subnet_key].id) : var.vnets[try(var.settings.vnet.lz_key, var.settings.lz_key, var.client_config.landingzone_key)][try(var.settings.vnet.key, var.settings.vnet_key)].subnets[try(var.settings.default_node_pool.subnet_key, var.settings.default_node_pool.subnet.key)].id
...
resource "azurerm_kubernetes_cluster_node_pool" "nodepools" {
  ...
  # TODO: add virtual subnet key
  vnet_subnet_id = can(each.value.subnet.resource_id) || can(each.value.vnet_subnet_id) || can(each.value.virtual_subnet_key) ? try(each.value.subnet.resource_id, each.value.vnet_subnet_id, var.virtual_subnets[try(each.value.lz_key, var.client_config.landingzone_key)][each.value.virtual_subnet_key].id) : var.vnets[try(var.settings.vnet.lz_key, var.settings.lz_key, var.client_config.landingzone_key)][try(var.settings.vnet.key, var.settings.vnet_key)].subnets[try(each.value.subnet.key, each.value.subnet_key)].id
