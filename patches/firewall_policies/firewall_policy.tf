...
resource "azurerm_firewall_policy" "fwpol" {
  ...
  #TODO: tls inspection
  # tls_certificate{
  #     key_vault_secret_id = try(var.settings.tls_certificate_key_vault_secret_id, null) # azurerm_key_vault_secret.Certificate.id//<id of the keyvault Secret where CA is stored>
  #     name = try(var.settings.tls_certificate_name, null) # //<name of the certificate stored in the keyvault>
  # }
  dynamic "tls_certificate" {
    for_each = try(var.settings.tls_certificate, null) == null ? [] : [1]

    content {
      key_vault_secret_id = try(var.settings.tls_certificate.key_vault_secret_id, null)
      name        = try(var.settings.tls_certificate.name, null)
    }
  } 
