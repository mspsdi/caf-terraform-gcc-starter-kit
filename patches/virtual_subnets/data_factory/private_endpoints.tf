module "private_endpoint" {
  ...
  resource_group_name = var.remote_objects.resource_groups[try(var.client_config.landingzone_key)][each.value.resource_group_key].name
  ...
  subnet_id           = can(each.value.subnet_id) ? each.value.subnet_id : try(var.remote_objects.vnets[try(each.value.lz_key, var.client_config.landingzone_key)][each.value.vnet_key].subnets[each.value.subnet_key].id,var.virtual_subnets[try(each.value.lz_key, var.client_config.landingzone_key)][each.value.virtual_subnet_key].id)
