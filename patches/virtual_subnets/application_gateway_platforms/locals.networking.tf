  ...
  ip_configuration = {
    ...
    private = {
      subnet_id = try(coalesce(
        ...
        , try(var.virtual_subnets[try(var.settings.front_end_ip_configurations.private.lz_key, var.client_config.landingzone_key)][var.settings.front_end_ip_configurations.private.virtual_subnet_key].id,null)
      ), null)
      cidr = try(coalesce(
